...
import com.greencross.gctemperlib.greencare.network.tr.data.Tr_meal_input_food_edit_data;
    ...
    public void insert(List<Tr_get_meal_input_food_data.ReceiveDatas> datas) {
        ...
        try {
            for (Tr_get_meal_input_food_data.ReceiveDatas data : datas) {
                ...
                cv.put(FOOD_FOODCODE, data.foodcode);
                cv.put(FOOD_FORPEOPLE, data.forpeople);
                cv.put(FOOD_REGDATE, CDateUtil.getRegDateFormat_yyyyMMddHHss(data.regdate));
                db.insert(TB_DATA_FOOD_DETAIL, null, cv);
...
//                mHelper.transactionExcute(sb.toString());
    ...
    */
    public List<DBHelperFoodCalorie.Data> getFoodList(String idx) {
        ...
        String sql = "Select "
                + " code, kind, name, tb_data_food_detail.forpeople, gram, unit, calorie, carbohydrate, protein, fat, sugars, salt, cholesterol, saturated, ctransquantic "
