    ...
    public void insert(Tr_get_hedctdata datas, boolean isServerReg) {
        ...
        try {
            ...
            for (Tr_get_hedctdata.DataList data : datas.data_list) {
                ...
                cv.put(PPG_IDX, data.idx);
                cv.put(PPG_HRM, data.hrm);
                cv.put(PPG_REGTYPE, data.regtype);
                cv.put(IS_SERVER_REGIST, isServerReg);
                cv.put(PPG_REGDATE, CDateUtil.getRegDateFormat_yyyyMMddHHss(data.reg_de));
                db.insert(TB_DATA_PPG, null, cv);
...
//        for (BandModel model : models) {
    ...
    // 최근 심박수
     */
    public PPGValue getResultFastPPG(DBHelper helper, String sDate, String eDate) {
