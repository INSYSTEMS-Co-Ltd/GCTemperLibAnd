    ...
    public void insert(Tr_get_hedctdata datas, boolean isServerReg) {
        ...
        try {
            ...
            for (Tr_get_hedctdata.DataList data : datas.data_list) {
                ...
                cv.put(SUGAR_IDX, data.idx);
                cv.put(SUGAR_SUGAR, data.sugar);
                cv.put(SUGAR_HILOW, data.hilow);
                cv.put(SUGAR_BEFORE, data.before);
                cv.put(SUGAR_DRUGNAME, data.drugname);
                cv.put(SUGAR_REGTYPE, data.regtype);
                cv.put(IS_SERVER_REGIST, isServerReg);
                cv.put(SUGAR_REGDATE, CDateUtil.getRegDateFormat_yyyyMMddHHss(data.reg_de));
                db.insert(TB_DATA_SUGAR, null, cv);
...
//                mHelper.transactionExcute(sb.toString());
    ...
    public String[] getResultMain(SQLiteOpenHelper helper) {
        ...
        // DB에 있는 데이터를 쉽게 처리하기 위해 Cursor를 사용하여 테이블에 있는 모든 데이터 출력
        String sql = "SELECT " + SUGAR_IDX + ", " + SUGAR_SUGAR + "," + SUGAR_BEFORE + ", "+SUGAR_REGDATE +
                " FROM " + TB_DATA_SUGAR +
                " WHERE " + SUGAR_DRUGNAME + " = ''" +
                " Order by datetime("+ SUGAR_REGDATE +") desc, cast(" + SUGAR_IDX + " as BIGINT) DESC LIMIT 1;";
        Logger.i(TAG, "sugarMainSql=" + sql);

        Cursor cursor = db.rawQuery(sql, null);
        if (cursor == null) {
            Logger.e(TAG, "sugarResultMain cursor == null");
            return null;
        }

        if (!cursor.moveToFirst()) {
            cursor.close();
            Logger.e(TAG, "sugarResultMain !cursor.moveToFirst()");
            return null;
        }
        Logger.i(TAG, "sugarResultRecent cursor.moveToNext():" + cursor.getCount());
        if (cursor.getCount() > 0) {
            try {
                String sugarIdx = cursor.getString(cursor.getColumnIndex(SUGAR_IDX));
                String regdate = cursor.getString(cursor.getColumnIndex(SUGAR_REGDATE));
                String type = cursor.getString(cursor.getColumnIndex(SUGAR_BEFORE));   // 식전 식후
                String sugar = cursor.getString(cursor.getColumnIndex(SUGAR_SUGAR));
                Logger.i(TAG, "result SUGAR_REGDATE[" + regdate + "], sugar=" + sugar+", sugarType="+type);
