    ...
    public void insert(List<Tr_get_hedctdata.DataList> datas, boolean isServerReg) {
        ...
        try {
            for (Tr_get_hedctdata.DataList data : datas) {
                ...
                cv.put(WATER_IDX, data.idx);
                cv.put(WATER_AMOUNT, data.amount);
                cv.put(WATER_REGTYPE, data.regtype);
                cv.put(IS_SERVER_REGIST, isServerReg);
                cv.put(WATER_REGDATE, CDateUtil.getRegDateFormat_yyyyMMddHHss(data.reg_de));
                db.insert(TB_DATA_WATER, null, cv);
